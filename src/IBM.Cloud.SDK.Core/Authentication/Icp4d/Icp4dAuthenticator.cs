...
namespace IBM.Cloud.SDK.Core.Authentication.Icp4d
{
    ...
    public class Icp4dAuthenticator : Authenticator
    {
        ...
        private static string ErrorMsg = "Error while retrieving access token from ICP4D token service: {0}";
        ...
        protected string GetToken()
        {
            ...
            if(!string.IsNullOrEmpty(config.GetUserManagedAccessToken()))
            {
            ...
            else
            {
                ...
                if (this.tokenData == null || !this.tokenData.IsTokenInvalid())
                {
